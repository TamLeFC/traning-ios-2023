...
class AppNetworkImp: AppNetwork {
    ...
    func requestWithoutMapping(_ target: AppApi) -> Single<Moya.Response> {
        return provider.request(target).observe(on: MainScheduler.instance)
    }
    
    func request(_ target: AppApi) -> Single<Any> {
        return provider.request(target).mapJSON().observe(on: MainScheduler.instance)
    }
    
    func requestObject<T: BaseMappable>(_ target: AppApi, type: T.Type) -> Single<T> {
        return provider.request(target).mapJSON().map { Mapper<T>().map(JSONObject: $0)}
